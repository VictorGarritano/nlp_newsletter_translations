...
- Também foi introduzido nessa semanas os [Notebooks](https://github.com/dair-ai/notebooks), focando no compartilhamento de *notebooks* de Ciência de Dados com a comunidade. Se você tem algum que gostaria de compartilhar, entre em contato conosco!
...
\\
Esse [artigo](https://arxiv.org/abs/2003.07278v1) fornece um compilado de metodologias para o aprendizado de *embeddings* contextualizados. Também estão inclusos uma revisão dos casos de usos da técnica para *transfer learning*, métodos de compressão de modelos e análises.
...
E aqui está outra [coletânea](https://arxiv.org/pdf/2003.08271.pdf) de modelos de linguagem pré-treinados, que propõe uma taxonomia para modelos pré-treinados de NLP.
...
\\
Diversos trabalhos demonstram como o *meta-learning* pode viabilizar a adoção de técnicas de *Deep Learning* para melhorar *benchmarks* de *few-shot learning*. Essa ideia é particularmente útil quando nos deparamos com situações onde a quantidade de dados disponíveis é limitada, como no caso de desenvolvimento de novos medicamentos. Um [artigo recente](https://arxiv.org/abs/2003.05996) aplicou uma técnica de *meta-learning* denominada *Model-Agnostic-Meta-Learning (MAML)*, assim como suas variantes, para predizer propriedades químicas em cenários de escassez de dados. Os resultados obtidos demonstraram que a abordagem utilizada tem um desempenho similar a outros métodos multi-tarefa pré-treinados.
...
\\
Um trabalho bastante interessante envolvendo pesquisadores da UC Berkeley, Google Research e da UC San Diego desenvolveu um método ([NeRF](http://www.matthewtancik.com/nerf)) para a criação de novas perspectivas em cenários complexos. Tomando um conjunto de imagens RGB como base de dados, o modelo utiliza coordenadas 5D (espaço, localização e direção) para o treinamento de uma rede neural profunda totalmente conectada, otimização uma *continuous volumetric scene function*, e retorna a densidade de volume e radiância para aquela localização. Os diversos valores de saída são combinados ao longo de um *camera ray* e renderizados como *pixels*, Essas saídas renderizadas são utilizadas para otimizar representações de cenas através da minimização do erro de renderização para todos os *camera rays* das imagens RGB. Comparada a outras abordagens para a tarefa, a NeRF é quantitativa e qualitativamente melhor e consegue resolver algumas inconsistências como a ausência de pequenos detalhes e *flickering* indesejado. 
